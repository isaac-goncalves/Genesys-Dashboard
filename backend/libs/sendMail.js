const nodemailer = require("nodemailer");
const UsersFrontend = require("../../sequelize/models/UsersFrontend");
const writeXlsxFile = require('write-excel-file/node')
const date = new Date;
//get date in the format dd-YY-YYYY
const days = date.getDate()
const month = date.getMonth() + 1
const year = date.getFullYear()
const fullDate = `${days < 10 ? '0' + days : days}${month < 10 ? '0' + month : month}${year}`

console.log(fullDate)

const schema = [
  {
    column: 'Nome',
    type: String,
    value: user => user.name,
    width: 25
  },
  {
    column: 'Último Login',
    type: String,
    // format: 'mm/dd/yyyy',
    value: user => user.datelastlogin,
    width: 20
  },
  {
    column: 'Departamento',
    type: String,
    value: user => user.department,
    width: 15
  },
  {
    column: 'Gerente',
    type: String,
    value: user => user.manager,
    width: 25
  },
  {
    column: 'Licença',
    type: String,
    value: user => user.license
  }
]

const smtpTransport = nodemailer.createTransport({
  host: "156.147.1.150",
  port: 25,
  auth: {
    user: "",
    pass: "",
  },
});

async function sendMail() {

  let userNames = [];

  const user = await UsersFrontend.findAll({
    // limit: 50
    order: [
      ['datelastlogin', 'ASC'],
  ]
  });

  const userFiltered = JSON.stringify(user, null, 2)
  const parsed = JSON.parse(userFiltered)

  //compare to se if date is smaller than 30 days
  const filteredusers = parsed.filter(user => {
    const datelastlogin = user.datelastlogin
    const date = new Date(datelastlogin);
    const today = new Date();
    const diffTime = Math.abs(today - date);
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    // console.log(diffDays);
    if (diffDays > 30 && user.license == "cloudCX3" && user.name != "WALLBOARD 1" && user.name != "WALLBOARD 2" && user.name != "WALLBOARD 3" && user.name != "WALLBOARD 4" && user.name != "WALLBOARD 5") {
      return user
    }
  })
  filteredusers.forEach(user => {
    userNames = [...userNames, user.name]
    console.log("Usuário: " + user.name + " - " + user.datelastlogin)
  })

  await writeXlsxFile(filteredusers, {
    schema,
    filePath: '../excelExport/file.xlsx'
  })
  console.log("XLS File created!")

  //send email

  const data =

    await smtpTransport.sendMail({
      from: "genesys-monitoring@lge.com",
      to: "isaac.goncalves1@lgcns.com;",
      subject: "[Genesys] Usuários Inativos",
      text: "email",
      html: `<p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'>Foi detectado que o usu&aacute;rios<strong><span style='font-family:"Calibri",sans-serif;'>,</span></strong></p>
    <p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'></p>
    <p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'><strong><span style='font-family:"Calibri",sans-serif;'><strong><span style='font-family:"Calibri",sans-serif;'>${userNames.join("<br>")}</span></strong></span></strong></p>
    <p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'><br></p>
    <p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'><span style='font-family:"Calibri",sans-serif;'><span style='font-family:"Calibri",sans-serif;'>Fi</span><span style='font-family:"Calibri",sans-serif;'>zeram o ultimo login a mais de 30 dias.&nbsp;</span></span></p>
    <p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'><br></p>
    <p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'>Segue em anexo a planilha com mais detalhes.</p>
    <p style='margin-right:0cm;margin-left:0cm;font-size:15px;font-family:"Calibri",sans-serif;margin:0cm;margin-bottom:.0001pt;'><br></p>
    <p><strong><img src="data:image/png;base64,"></strong></p>
    <p><strong><br></strong></p>
    <p><strong><br></strong></p>
    <p><br></p>`,
      attachments: [
        {   // file on disk as an attachment
          filename: `Usuarios_Inativos_${fullDate}.xlsx`,
          path: '../excelExport/file.xlsx' // stream this file
        }
      ]
    }, (err) => {
      if(err) console.log(err)
      else{
        console.log("Mail Sent")
      }
    })
}

sendMail()

module.exports = { sendMail }
